# -*- coding: utf-8 -*-
"""
Created on Thu Apr 12 20:20:58 2018
...
class BPNN:
    ...
    def __init__(self,input_node_num,hidden_node_num,output_node_num):
        ...
        self.w_h_o = self.utils.create_random_matrix(self.h_n,self.o_n,0.2)
    ...
    #训练
    def train(self,hidden_activation,output_activation,train_inputs,train_outputs,test_inputs,test_outputs,alpha,error_threshold,iteration_num,batch_size):
        #隐含层的激活函数
    ...
    def batch(self):
        ...
        while True:
            ...
            if self.iteration_num%100 == 0:
                self.test(self.test_inputs,self.test_outputs)
            ##自适应调整学习速率alpha
...
#        self.train_error = self.train_error + (np.transpose(tmp)*tmp)[0,0]
    ...
    #更新权值W
    def update(self,inputs_indexs):
        #按照公式更新输入层与隐含层之间的w: w^{T-2}_{kj}:=w^{T-2}_{kj}-\alpha\frac{1}{p}\sum_{i=1}^{p}\Delta_{ij}^{T-2}\cdot o^{T-2}_{ik}
        w_i_h_bp = self.w_i_h - self.alpha/self.batch_size*self.w_i_h_delta
        #按照公式更新隐含层与输出层之间的w: w^{T-1}_{kj}:=w^{T-1}_{kj}-\alpha\frac{1}{p}\sum_{i=1}^{p}\Delta_{ij}^{T-1} \cdot o^{T-1}_{ik}
        w_h_o_bp = self.w_h_o - self.alpha/self.batch_size*self.w_h_o_delta
        #delta清零
        self.w_i_h_delta = Utils().matrix_fill_zeros(self.w_i_h_delta)
        self.w_h_o_delta = Utils().matrix_fill_zeros(self.w_h_o_delta)
        #random search
        w_i_h_rs = np.random.uniform(-0.2,0.2,(self.i_n,self.h_n))
        w_h_o_rs = np.random.uniform(-0.2,0.2,(self.h_n,self.o_n))
        #比较两种更新得到的训练误差
        error_bp = self.error_calculation(w_i_h_bp,w_h_o_bp,self.train_inputs,self.train_outputs)
        error_rs = self.error_calculation(w_i_h_rs,w_h_o_rs,self.train_inputs,self.train_outputs)
        ##计算最小批的误差
#        error_bp = 0
#        error_rs = 0
#        for i in inputs_indexs:
#            error_bp += self.error_cal(w_i_h_bp,w_h_o_bp,i)
#            error_rs += self.error_cal(w_i_h_rs,w_h_o_rs,i)
#        error_bp = error_bp/self.batch_size
#        error_rs = error_rs/self.batch_size
        if error_rs < self.error-self.epsilon and error_rs < error_bp:
            self.w_i_h = w_i_h_rs;self.w_h_o = w_h_o_rs; self.error = error_rs; self.error_store.append(error_rs)
            print('rs')
        elif error_bp < self.error-self.epsilon and error_bp < error_rs:
            self.w_i_h = w_i_h_bp; self.w_h_o = w_h_o_bp; self.error = error_bp; self.error_store.append(error_bp)
            #print('bp')
        else:
            self.error_store.append(self.error)
            #print('Unchanged')
    ...
    def terminate(self):
        ...
        if self.iteration_num <= 0:
            ...
            #print ("error: ",self.train_error)
            open('error.txt','a').write('\n\terror:%f'%self.train_error)
            self.train_error = 0.0
            return False
    ...
    def error_cal(self,w_i_h,w_h_o,item):
        h_outputs = self.h_activation.f(np.dot(self.train_inputs[item,:],w_i_h))
        o_outputs = self.o_activation.f(np.dot(h_outputs,w_h_o))
        error_train = self.train_outputs[item,:] - o_outputs
        train_error_temp = 0.5*(np.linalg.norm(error_train))**2
        return train_error_temp
...
if __name__=="__main__":
    ...
    bpnn.train(Sigmoid(), Sigmoid(),input_matrix,output_matrix,test_input,test_output, 0.2, 1e-8,100, 50000)
    end_time = time.time()
